...
var jason = require('ezjason');
var package = jason.read('package.json');
...
var inventory = {
    ...
    has: function invenHas(item) {
        var a = {};
        player.inventory.forEach(function (el) {
            if (el.id == item) {
                a = el;
            }
        });
    ...
    clean: function cleanInventory() {
        ...
        for (var i = 0; i < player.inventory.length; i++) {
            ...
            if (typeof el['amount'] === 'string') {
                var finalArray = [];
                el['amount'].split('-').forEach(function (amountSplit) {
...
function pre() {
    ...
    dns.resolve('www.google.com', function (e) {
        if (e) {
            ...
            nodefetch('https://raw.githubusercontent.com/cmexdev/PATH/main/package.json', { method: 'Get' }).then(function (res) { return res.json(); }).then(function (json) {
                    ...
                    term.singleLineMenu(['Update now', 'Cancel'], function (e, r) {
                        if (e) {
                            ...
                            if (r.selectedIndex == 1) {
                                ...
                                var install_1 = fs.createWriteStream(filename);
                                var request = http.get('http://i3.ytimg.com/vi/J---aiyznGQ/mqdefault.jpg', function (r) {
                                    r.pipe(install_1);
                                });
                            }
                        }
...
function loadGame() {
    ...
    gamesFolder.forEach(function (game) {
        gamesList.push(game);
    });
...
function game(task, game, enviroid, ignorechests) {
    ...
    if (task == 'randomgen') {
        ...
        var environment = environments[randomNumber];
        var environmentName = environment.split('.json')[0];
        var environmentData = jason.read('game\\environments\\' + environment);
        ...
        var environmentVariant_1 = environmentData.variants[randomNumber];
        gamedata.state = ['enviro', environment.split('.json')[0], environmentVariant_1.id];
        saveGame(environmentName, environmentVariant_1.id, gamedata.currentgame, player.inventory, player.level, player.discovered, gamedata.state);
        console.log(boxen(environmentVariant_1.message, {
            padding: 1,
            borderStyle: 'round',
            borderColor: 'gray'
        }));
        var options = [];
        var arr = 0;
        environmentVariant_1.options.forEach(function (currentOption) {
            arr++;
        ...
        term.singleColumnMenu(options, menuOptions, function (e, selectedOption) {
            if (e) {
                ...
                if (selectedOption.selectedIndex != 3) {
                    action(environmentVariant_1.options[selectedOption.selectedIndex].action);
                }
                else {
                    action('show:options');
                }
    ...
    //* requires 'task', 'game', 'ignorechests'
        ...
        var environment_1 = game;
        var environmentData = jason.read('game\\environments\\' + environment_1 + '.json');
        //Get random variant
        var lengthOfEvironmentVariants = environmentData.variants.length;
        var randomNumber = Math.floor(Math.random() * lengthOfEvironmentVariants);
        //Variant
        var environmentVariant_2 = environmentData.variants[randomNumber];
        gamedata.state = ['enviro', environment_1, environmentVariant_2.id];
        var playerDiscoveredBiome = 0;
        player.discovered.biomes.forEach(function (el) {
            if (el == environment_1) {
        ...
        if (playerDiscoveredBiome == 0) {
            ...
            levelUp('You discovered a new biome! (' + environment_1 + ')');
            player.discovered.biomes.push(environment_1);
            player.level += gamedata.levels.biomes[environment_1];
        }
        saveGame(environment_1, environmentVariant_2.id, gamedata.currentgame, player.inventory, player.level, player.discovered, gamedata.state);
        //SPAWN MONSTER?
        var spawnMonster = 0;
        gamedata.spawning.monster.biome.forEach(function (el) {
            if (el == environment_1) {
        ...
        if (spawnMonster > 0) {
            return spawn('monster', gamedata.currentgame, 'spawnrandom', null, [environment_1, environmentVariant_2.id]);
        }
        if (ignorechests != true) {
            return chestInit(gamedata.currentgame, gamedata.state);
        }
        console.log(boxen(environmentVariant_2.message, {
            padding: 1,
            borderStyle: 'round',
            borderColor: 'gray'
        }));
        var options = [];
        var arr = 0;
        environmentVariant_2.options.forEach(function (el) {
            arr++;
        ...
        term.singleColumnMenu(options, menuOptions, function (e, resp) {
            if (e) {
                ...
                if (resp.selectedIndex != 3) {
                    action(environmentVariant_2.options[resp.selectedIndex].action);
                }
                else {
                    action('show:options');
                }
    ...
    //* requires 'task', 'game', 'enviroid'
        ...
        var environment_2 = game;
        var variantId = enviroid;
        gamedata.state = ['enviro', environment_2, variantId];
        ...
        try {
            environmentVariant = jason.read('game\\environments\\' + environment_2 + '.json');
        }
        catch (er) {
        ...
        for (var i = 0; i < environmentVariant.variants.length; i++) {
            var el = environmentVariant.variants[i];
            if (el.id == variantId) {
                index = i;
            }
        }
        var en_1 = environmentVariant.variants[index];
        var playerDiscoveredBiome = 0;
        player.discovered.biomes.forEach(function (el) {
            if (el == environment_2) {
                playerDiscoveredBiome++;
        ...
        if (playerDiscoveredBiome == 0) {
            //player has not discovered biome
            levelUp('You discovered a new biome! (' + environment_2 + ')');
            player.discovered.biomes.push(environment_2);
            player.level += gamedata.levels.biomes[environment_2];
        }
        saveGame(environment_2, variantId, gamedata.currentgame, player.inventory, player.level, player.discovered, gamedata.state);
        console.log(boxen(en_1.message, {
            padding: 1,
            borderColor: 'grey',
            borderStyle: 'round'
        }));
        var options = [];
        var arr = 0;
        en_1.options.forEach(function (el) {
            arr++;
            options.push(el.title);
        });
        options.push('More options');
        term.singleColumnMenu(options, menuOptions, function (e, resp) {
            if (e) {
                ...
                if (resp.selectedIndex != 3) {
                    action(en_1.options[resp.selectedIndex].action);
                }
                else {
                    action('show:options');
                }
...
function action(act) {
    ...
    if (task == 'go') {
        ...
        var itemToTake_1 = actionSplit[1].split('&')[0];
        var amountToTake = actionSplit[1].split('&')[1];
        var amount_1 = new Number(amountToTake).valueOf();
        player.inventory.forEach(function (item) {
            if (item.id == itemToTake_1) {
                item.amount -= amount_1;
            }
        });
        inventory.clean();
    }
        ...
        if (actionSplit[1].includes('=>') == false) {
            var itemToGive_1 = actionSplit[1].split('&')[0]; //* AS itemed
            var amountToTake = actionSplit[1].split('&')[1];
            var amount = new Number(amountToTake).valueOf();
            var amountStr = amount.toString();
            var float_1 = parseFloat(amountStr);
            var inventoryHasItem = 0;
            player.inventory.forEach(function (el) {
                if (el.id == itemToGive_1) {
                    inventoryHasItem++;
                    el.amount += float_1;
                }
            });
...
function listItems() {
    var items = jason.read('game\\items.json');
...
function showOptions(gamed) {
    var gameData = jason.read('game\\games\\' + gamed + '\\game.json');
                                ...
                                term.singleColumnMenu(sellingInventoryOptions, function (e, r) {
                                    if (e) {
                                        ...
                                        if (r.selectedIndex == sellingInventoryTable.length - 1) {
                                            ...
                                            var itemid_1 = sellReference[r.selectedText]; //* array: 0=itemid, 1=amountofitem
                                            var item = gamedata.items.items[itemid_1[0]];
                                            var itemworth_1 = item.worth;
                                            ...
                                            term.singleLineMenu(['All of it', 'Half of it', 'A specific amount', 'Cancel'], function (e, r) {
                                                ...
                                                if (r.selectedIndex == 0) {
                                                    ...
                                                    var itemtotake = itemid_1[0];
                                                    player.inventory.forEach(function (el) {
                                                        if (el.id === itemtotake) {
                                                            el.amount += -itemid_1[1];
                                                            player.level += itemworth_1 * itemid_1[1];
                                                        }
                                                    });
                                                    console.log('\n\nYou ' + colors.underline('sold') + ' all of your ' + itemtotake.toString().underline + '!');
                                                    console.log('You gained ' + colors.underline((itemworth_1 * itemid_1[1]).toString()) + ' levels!');
                                                    inventory.clean();
                                                    ...
                                                    player.inventory.forEach(function (item) {
                                                        if (item.id === itemtotake) {
                                                            item.amount += -(itemid_1[1] / 2);
                                                            player.level += itemworth_1 * (itemid_1[1] / 2);
                                                        }
                                                    });
                                                    console.log('\n\nYou ' + colors.underline('sold') + ' half of your ' + itemtotake.toString().underline + '!');
                                                    console.log('You gained ' + colors.underline((itemworth_1 * (itemid_1[1] / 2)).toString()) + ' levels!');
                                                    inventory.clean();
                                                    term.singleLineMenu(['OK'], function (e, r) {
                                                        if (gameData.gamestate[0] == 'mob') {
                                                            return spawn(gameData.gamestate[3], gameData.name, 'spawncontinue', gameData.gamestate[1], gameData.gamestate[2]);
                                                        ...
                                                        if (e) {
                                                            ...
                                                            if (r != undefined) {
                                                                ...
                                                                player.inventory.forEach(function (el) {
                                                                    if (el.id === itemToTake) {
                                                                        el.amount += -num;
                                                                        player.level += itemworth_1 * num;
                                                                    }
                                                                });
                                                                console.log('\n\nYou ' + colors.underline('sold') + ' ' + num + ' of your ' + itemToTake.toString().underline + '!');
                                                                console.log('You gained ' + colors.underline((itemworth_1 * num).toString()) + ' levels!');
                                                                inventory.clean();
                                                                term.singleLineMenu(['OK'], function (e, r) {
                                                                    if (gameData.gamestate[0] == 'mob') {
                                                                        return spawn(gameData.gamestate[3], gameData.name, 'spawncontinue', gameData.gamestate[1], gameData.gamestate[2]);
...
function spawn(mobtype, gn, task, mob, gamed) {
    if (task == 'spawnrandom') {
        if (mobtype == 'monster') {
            var arr = 0;
            var monsterMobList = [];
            gamedata.mob.forEach(function (mob) {
                if (mob.type == 'monster') {
                ...
                if (e) {
                    ...
                    if (r.selectedIndex == 0) {
                        ...
                        if (inventoryHasTypeToDefeat[0] == true) {
                            ...
                            inventoryHasTypeToDefeat[1].forEach(function (item) {
        ...
        if (mobtype == 'monster') {
            var randomMonster = {};
            gamedata.mob.forEach(function (el) {
            ...
            console.log(colors.cyan('Prepare to attack!'));
            var options = [colors.magenta('rUn aWaY'), 'Prepare', 'More options'];
            term.singleLineMenu(options, function (e, r) {
                if (e) {
                    err(e);
...
function options(task, option) {
    ...
    if (task == 'frommenu') {
        ...
        if (option == 'managegames') {
            ...
            var games_1 = fs.readdirSync('game\\games');
            var g_1 = [colors.bgBlue('Return to options menu')];
            games_1.forEach(function (el) {
                g_1.push(el);
            });
            term.singleColumnMenu(g_1, function (e, r) {
                if (e) {
                    err(e);
                    //todo add err code
                }
                    ...
                    if (r.selectedIndex == 0) {
                        ...
                        console.clear();
                        var sg = games_1[r.selectedIndex - 1];
...
function craft(gamed) {
    console.clear();
    var items = gamedata.items.items;
    var list = gamedata.items.list;
    var g = gamed;
    ...
    term.singleColumnMenu(array, function (e, r) {
        if (e) {
            ...
            if (r.selectedText != colors.bgRed('Cancel')) {
                var el = ref[r.selectedIndex][r.selectedIndex];
